...
matfile_folder =  dir("D:\KWAP_true_old\matfile1216\*.mat");
...
for id = 1:9 % 1:9
    ...
    save_name = "Peak_1030_roujin" + id + "_front_plane";
    save_dir = "graph\1216\"+ save_name;
    mkdir("graph\1216\"+ save_name)
    ...
    %% 左右逆転の防止
    joints_No = [10 11 12 13 14 15 26 27 9]; % Rhip, Knee, Ankle, LHip, Knee, Ankle, RKnee-Ankle, LKnee-Ankle, MidHip
...
%     joints_No = [10 11 12 13 14 15 9]; % Rhip, Knee, Ankle, LHip, Knee, Ankle, MidHip
    ...
    % AKからest_ankleを算出
    Rankle_est = f_Joints(:,4) - (f_Joints(:,4) - f_Joints(:,19))*2;
    Lankle_est = f_Joints(:,13) - (f_Joints(:,13) - f_Joints(:,22))*2;
    
    
    % 右足の骨盤・腰・膝・足首・1/2の時系列
    
    f = figure(1);
    % 背景色
    set(gcf, 'Color', 'none')
    t = 0:0.016666:(size(poses3d,1)-1)*0.016666;
    plot(t, f_Joints(:,4), t, f_Joints(:,7), t, f_Joints(:,19), t, Rankle_est);
...
%     legend("RKnee","RAnkle","RAK","EstAnkle",'Location','southeast')
    legend("RKnee","RAnkle","RAK","EstAnkle",'Location','best')

    sheet_name = ["Rposition"];
    filename = save_dir + "\" + sheet_name(1);
    ...
    set(gcf, 'Color', 'none')
    t = 0:0.016666:(size(poses3d,1)-1)*0.016666;
    plot(t, f_Joints(:,13), t, f_Joints(:,16), t, f_Joints(:,22), t, Lankle_est);
...
%     h2 = xline(R_off, '--r', 'R-off');
    ...
    legend("LKnee","LAnkle","LAK","EstAnkle",'Location','best')
%     legend("LKnee","LAnkle","LAK","EstAnkle",'Location','southeast')

    sheet_name = ["Lposition"];
    filename = save_dir + "\" + sheet_name(1);
    saveas(gcf,filename,'svg')
    hold off
    ...
    for i = 1:8
        try            
            Joint_diff(:,i*3-2) = f_Joints(:,i*3-2)-f_Joints(:,25);
            Joint_diff(:,i*3-1) = f_Joints(:,i*3-1)-f_Joints(:,26);
            Joint_diff(:,i*3  ) = f_Joints(:,i*3  )-f_Joints(:,27);
        catch

        end
    end
    estRA_diff = Rankle_est - f_Joints(:,25);
    estLA_diff = Lankle_est - f_Joints(:,25);
    
    f = figure(3);
    set(gcf, 'Color', 'none')
    plot(t, Joint_diff(:,4),t, Joint_diff(:,7),t, Joint_diff(:,19),t,estRA_diff)
    h1 = xline(R_on, '-r', 'R-on');
    h2 = xline(R_off, '--r', 'R-off');
    xlabel('time [s]') 
    ylabel('position [m]') 
...
%     legend("RKnee","RAnkle","RAK","EstAnkle",'Location','southeast')
    ...
    f = figure(4);
    set(gcf, 'Color', 'none')
    plot(t, Joint_diff(:,13),t, Joint_diff(:,16),t, Joint_diff(:,22),t,estLA_diff)
    h3 = xline(L_on, '-b', 'L-on');
    h4 = xline(L_off, '--b', 'L-off');
    xlabel('time [s]') 
    ylabel('position [m]') 
...
%     legend("LKnee","LAnkle","LAK","EstAnkle",'Location','southeast')
    ...
    %% 関節同士の差分
    try            
        same_diff(:,1) = f_Joints(:,4)-f_Joints(:,13); % Knee
        same_diff(:,2) = f_Joints(:,7)-f_Joints(:,16); % Ankle
        same_diff(:,3) = f_Joints(:,19)-f_Joints(:,22);% AK       
    catch
    ...
    f = figure(5);
    set(gcf, 'Color', 'none')
    plot(t, same_diff(:,1),t, same_diff(:,2),t, same_diff(:,3))
    ...
    legend("Knee","Ankle","AK",'Location','southeast')

    sheet_name = ["jointdiff"];
    filename = save_dir + "\" + sheet_name(1);
    saveas(gcf,filename,'svg')
    hold off
    ...
    plot(t, f_Joints(:,1) - f_Joints(:,10), t, detrend_sdata)
    ...
    legend("Hipdiff",'Location','southeast')
...
% %         L2_joint_diff = (Joint_diff(:,i*3-2).^2 + Joint_diff(:,i*3-1).^2 + Joint_diff(:,i*3).^2).^(1/2)
% %         plot(t, Joint_diff(:,i*3-2), t, L2_joint_diff)
%         hold on
...
% 
%         sheet_name = ["diff_Rhip","diff_Rknee","diff_Rankle","diff_Lhip","diff_Lknee","diff_Lankle", "diff_RAK", "diff_LAK"];
% 
...
% 
%     try            
%         AK_diff(:,1) = f_Joints(:,19)-f_Joints(:,22);
%         AK_diff(:,2) = f_Joints(:,20)-f_Joints(:,23);
%         AK_diff(:,3) = f_Joints(:,21)-f_Joints(:,24);       
%     catch
% 
%     end
% 
...
% 
% %     findpeaks(knee_diff(:,1),t, 'MinPeakHeight',0.05, 'MinPeakDistance',0.5);
%     [pks_p,locs_p] = findpeaks( AK_diff(:,1), t, 'MinPeakDistance',0.5);
%     [pks_n,locs_n] = findpeaks(-AK_diff(:,1), t, 'MinPeakDistance',0.5);
%     plot(locs_p,pks_p,'rv','MarkerFaceColor','r');
%     plot(locs_n,-pks_n,'rv','MarkerFaceColor','b');
% 
%     h1 = xline(R_on, '-r', 'R-on');
...
%     legend("X","Y","Z",'Location','southeast')
% 
%     filename = "graph\withoutplane\" + save_name + "\" + "AK_diff";
%     saveas(gcf,filename,'svg')
%     hold off
